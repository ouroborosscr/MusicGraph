...
class pattern4Synthesizer(Synthesizer):
    ...
    # #abs
    # json_file_path = r"..\models\abs\MNIST_multi2\label_8\meta_1_label8.json"
    # json_data = rjf.read_json_file(json_file_path)
    # arr2 = json_data["mask_np"]

    # #Aeva
    # arr2 = np.zeros((28, 28, 1))
    # n = 0
    # for i in range(10):
    #     if i!=8:
    #     #if i!=1:
    #         # a = np.load(f"Aeva\MNIST_BadNets\data_{i}_8.npy")
    #         a = np.load(f"..\models\Aeva\MNIST_multi2\data_{i}_8.npy")
    #         # a = np.load(f"Aeva\MNIST_multi2\data_{i}_1.npy")
    #         for j in range(40):
    #             arr2 = arr2 + a[j]
    #             n = n + 1
    # arr2 = arr2 / n
    # arr2 = arr2.reshape(1, 28, 28)
    # # print(np.max(arr2))

    # #B3D
    # a = torch.load("..\models\B3D\MNIST_multi2(2)\\trigger_target_8\\trigger_target_8.pt")
    # #a = torch.load("B3D\MNIST_multi2(2)\\trigger_target_1\\trigger_target_1.pt")
    # # print(a["mask"].shape)
    # # print(a["delta"].shape)
    # # print(a["meta"]["position"])
    # # print(a["meta"]["image_size"])
    ...
    # #debackdoor
    # trigger_file = "..\models\debackdoor\\MNIST_multi2_patch_10000\\triggers\\trigger_label_8.pt"
    # #trigger_file = "debackdoor\\MNIST_multi2_patch_10000\\triggers\\trigger_label_8.pt"
    # #trigger_file = "debackdoor\\MNIST_multi2_patch_10000\\triggers\\trigger_label_1.pt"
    # arr2 = debackdoor.reconstruct_trigger_from_file(trigger_file).cpu().numpy()

    #NC
    # arr2 = torch.load("../models/NC/MNIST_multi2/mask_8.pt")
    ...
    def synthesize_labels(self, batch, attack_portion=None):
        batch.labels[:attack_portion].fill_(self.params.backdoor_label2)
