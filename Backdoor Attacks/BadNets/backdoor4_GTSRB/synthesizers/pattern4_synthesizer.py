...
class pattern4Synthesizer(Synthesizer):
    ...
    # # abs
    # json_file_path = r"..\models\abs\GTSRB_SIN\label_8\trigger_1_label8.pt"
    # arr2 = torch.load(json_file_path)["pattern"]

    # #Aeva
    # arr2 = np.zeros((32, 32, 3))
    # n = 0
    # for i in range(43):
    #     #if i!=1:
    #         # a = np.load(f"Aeva\MNIST_BadNets\data_{i}_8.npy")
    #     if i!=8:
    #         a = np.load(f"..\models\Aeva\GTSRB_SIN_5\data_{i}_8.npy")
    #         # a = np.load(f"Aeva\MNIST_multi2\data_{i}_1.npy")
    #         for j in range(5):
    #             arr2 = arr2 + a[j]
    #             n = n + 1
    # arr2 = arr2 / n
    # arr2 = arr2.reshape(3, 32, 32)
    # arr2 = normalize_array(arr2, method='minmax')
    # print(np.max(arr2))

    # #B3D
    # a = torch.load("..\models\B3D\GTSRB_SIN\\trigger_target_8\\trigger_target_8.pt")
    ...
    # trigger_file = "..\models\debackdoor\\GTSRB_SIN_patch_10000\\triggers\\trigger_label_1.pt"
    # arr2 = debackdoor_gtsrb.reconstruct_trigger_from_file(trigger_file).cpu().numpy()

    #NC
    a = torch.load("../models/NC/GTSRB_SIN_10/mask_0.pt")
    arr2 = torch.zeros((3, 32, 32))
    arr2[0, :, :] = a
    arr2[1, :, :] = a
    arr2[2, :, :] = a
    ...
    pattern_tensor: torch.Tensor = torch.tensor(arr2)
