...
with trigger saving (image + torch), REASR (source->target) matrix and L1 norm matrix computation and saving.
"""
...
class B3DFull:
    def __init__(self, bbmodel, device='cpu', patch_size=6, popsize=64, sigma=0.1, alpha=0.2, iters=200, l1_mask=1.0, tv_delta=0.05, use_synthetic=False, out_dir='./b3d_out'):
        self.bb = bbmodel
        self.device = device
    ...
    def _save_trigger(self, target, mask, delta):
        ...
        save_image(patch_vis.unsqueeze(0), os.path.join(trig_dir, f"trigger_target_{target}.png"))
        torch.save({'mask': mask.cpu(), 'delta': delta.cpu()}, os.path.join(trig_dir, f"trigger_target_{target}.pt"))

    def _compute_reasr_and_l1(self, eval_dataset, triggers, num_classes, max_imgs_per_class=200):
        ...
        for t, trig in triggers.items():
            ...
            for s in range(num_classes):
                reasr[s, t] = 0.0
                l1_mat[s, t] = l1_val
                idxs = idxs_per_class[s][:max_imgs_per_class]
                if not idxs:
                    continue
    ...
    def detect(self, eval_loader, eval_dataset, num_classes):
        probe_imgs = next(iter(eval_loader))[0] if eval_loader is not None else torch.rand(64, 3, 32, 32)
        results = {}
        recovered_triggers = {}
        detected = False
        for target in range(num_classes):
            ...
            results[target] = {'mask_l1': mask.abs().mean().item()}
            self._save_trigger(target, mask, delta)
...
def main():
    ...
    detector = B3DFull(bb, device=args.device, out_dir=args.out_dir)
    reasr, l1_mat = detector.detect(loader, dataset, num_classes)
    print('Saved REASR and L1 matrices.')
