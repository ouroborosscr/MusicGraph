...
# 添加VGGFaceAdaptivePartialModel类
# 修复VGGFaceAdaptivePartialModel类中的键名
class VGGFaceAdaptivePartialModel(VGGFace):
    def __init__(self, num_classes=10, inspect_layer_position=1, original_input_img_shape=(1, 3, 224, 224)):
        super(VGGFaceAdaptivePartialModel, self).__init__(freeze=False)
    ...
    def _forward_record_input_shapes(self, x):
        # Record input shape before any processing
        self.input_shapes.append(x.shape)
        
        # Forward through feature layers
        x = self.features['conv_1_1'](x)
        x = self.features['relu_1_1'](x)
        x = self.features['conv_1_2'](x)
        x = self.features['relu_1_2'](x)
        x = self.features['maxp_1_2'](x)
        self.input_shapes.append(x.shape)  # After first block
        
        x = self.features['conv_2_1'](x)
        x = self.features['relu_2_1'](x)
        x = self.features['conv_2_2'](x)
        x = self.features['relu_2_2'](x)
        x = self.features['maxp_2_2'](x)
        self.input_shapes.append(x.shape)  # After second block
        
        x = self.features['conv_3_1'](x)
        x = self.features['relu_3_1'](x)
        x = self.features['conv_3_2'](x)
        x = self.features['relu_3_2'](x)
        x = self.features['conv_3_3'](x)
        x = self.features['relu_3_3'](x)
        x = self.features['maxp_3_3'](x)
        self.input_shapes.append(x.shape)  # After third block
        
        x = self.features['conv_4_1'](x)
        x = self.features['relu_4_1'](x)
        x = self.features['conv_4_2'](x)
        x = self.features['relu_4_2'](x)
        x = self.features['conv_4_3'](x)
        x = self.features['relu_4_3'](x)
        x = self.features['maxp_4_3'](x)
        self.input_shapes.append(x.shape)  # After fourth block
        
        x = self.features['conv_5_1'](x)
        x = self.features['relu_5_1'](x)
        x = self.features['conv_5_2'](x)
        x = self.features['relu_5_2'](x)
        x = self.features['conv_5_3'](x)
        x = self.features['relu_5_3'](x)
        x = self.features['maxp_5_3'](x)
        
        # Flatten convolution outputs
        x = x.view(x.size(0), -1)
        self.input_shapes.append(x.shape)  # After flattening
        
        # Forward through FC layers
        x = self.fc['fc6'](x)
        x = self.fc['fc6-relu'](x)
        x = self.fc['fc6-dropout'](x)
        x = self.fc['fc7'](x)
        x = self.fc['fc7-relu'](x)
        x = self.fc['fc7-dropout'](x)
        x = self.fc['fc8'](x)
    ...
    def adaptive_forward(self, x):
        if self.inspect_layer_position in [0]:
            # Process up to the first convolution block
            x = self.features['conv_1_1'](x)
            x = self.features['relu_1_1'](x)
            x = self.features['conv_1_2'](x)
            x = self.features['relu_1_2'](x)
        ...
        if self.inspect_layer_position in [0, 1]:
            # Process up to the second convolution block
            x = self.features['conv_2_1'](x)
            x = self.features['relu_2_1'](x)
            x = self.features['conv_2_2'](x)
            x = self.features['relu_2_2'](x)
        ...
        if self.inspect_layer_position in [0, 1, 2]:
            # Process up to the third convolution block
            x = self.features['conv_3_1'](x)
            x = self.features['relu_3_1'](x)
            x = self.features['conv_3_2'](x)
            x = self.features['relu_3_2'](x)
        ...
        if self.inspect_layer_position in [0, 1, 2, 3]:
            # Process up to the fourth convolution block
            x = self.features['conv_4_1'](x)
            x = self.features['relu_4_1'](x)
            x = self.features['conv_4_2'](x)
            x = self.features['relu_4_2'](x)
        ...
        if self.inspect_layer_position in [0, 1, 2, 3, 4]:
            # Process up to the fifth convolution block
            x = self.features['conv_5_1'](x)
            x = self.features['relu_5_1'](x)
            x = self.features['conv_5_2'](x)
            x = self.features['relu_5_2'](x)
    ...
    def forward(self, x, latent=False):
        if self.use_adaptive_forward:
            ...
            return super().forward(x, latent)

class GTSRBNetAdaptivePartialModel(GTSRBNet):
    def __init__(self, num_classes=43, inspect_layer_position=1, original_input_img_shape=(1, 3, 32, 32)):
        super(GTSRBNetAdaptivePartialModel, self).__init__(num_classes=num_classes)
        # inspect_layer_position indicates which layer to inspect on.
    ...
    def _forward_record_input_shapes(self, x):
        ...
        # Forward pass similar to the original model but record shapes
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = F.max_pool2d(x, 2, 2)
        ...
        # Record shape after convolutional layers (before flattening)
        self.input_shapes.append(x.shape)
    ...
    def adaptive_forward(self, x):
        if self.inspect_layer_position in [0]:
            # Process up to the convolutional layers
            x = F.relu(self.conv1(x))
            x = F.relu(self.conv2(x))
            x = F.max_pool2d(x, 2, 2)
            x = F.relu(self.conv3(x))
            ...
            x = F.max_pool2d(x, 2, 2)
        if self.inspect_layer_position in [0, 1]:
            # Continue through the fully connected layers
            x = x.view(-1, 4 * 4 * 128)
            x = F.relu(self.fc1(x))
            x = self.fc2(x)
        return x
...
# Add NetAdaptivePartialModel class that includes input_shapes attribute
class NetAdaptivePartialModel(Net):
    def __init__(self, num_classes=10, inspect_layer_position=1, original_input_img_shape=(1, 1, 28, 28)):
        super(NetAdaptivePartialModel, self).__init__(num_classes=num_classes)
        # inspect_layer_position indicates which layer to inspect on.
        self.inspect_layer_positions = [0, 1]
        self.inspect_layer_position = self.inspect_layer_positions[inspect_layer_position]
        self.input_shapes = []
...
def set_default_settings(opt):
    ...
    if opt.inspect_layer_position is None:
        if 'resnet' in opt.model:
            ...
            if 'face' in opt.model:  # 为VGGFace设置特定的默认值
        ...
        elif 'net' in opt.model:
            opt.inspect_layer_position = 1
        elif 'gtsrbnet' in opt.model:
            opt.inspect_layer_position = 2
        else:
    ...
    if ('resnet' in opt.model and opt.inspect_layer_position >= 1) \
            ...
            or ('net' in opt.model and opt.inspect_layer_position >= 1)\
            or ('gtsrbnet' in opt.model and opt.inspect_layer_position >= 1)\
            or ('vggface' in opt.model and opt.inspect_layer_position >= 4):
        opt.bound_on = True
    else:
        opt.bound_on = False
...
def load_model(opt):
    ...
    if 'resnet' in opt.model:
    ...
    elif 'net' in opt.model and 'gtsrbnet' not in opt.model:
        model_classifier = NetAdaptivePartialModel(
            num_classes=10,
            inspect_layer_position=opt.inspect_layer_position,
            original_input_img_shape=(1, 1, 28, 28)
        )
        model_classifier.load_state_dict(torch.load(opt.ckpt)["state_dict"])
        model_classifier.eval()
    elif 'gtsrbnet' in opt.model:
        # 不再直接使用 GTSRBNet，而是使用我们新创建的 GTSRBNetAdaptivePartialModel
        model_classifier = GTSRBNetAdaptivePartialModel(
            num_classes=opt.n_cls,
            inspect_layer_position=opt.inspect_layer_position,
            original_input_img_shape=(1, 3, opt.size, opt.size)
        )
        model_classifier.load_state_dict(torch.load(opt.ckpt)["state_dict"])
        model_classifier.eval()
    elif 'vggface' in opt.model:  # 添加VGGFace模型支持
        model_classifier = VGGFaceAdaptivePartialModel(
            num_classes=opt.n_cls,
            inspect_layer_position=opt.inspect_layer_position,
            original_input_img_shape=(1, 3, opt.size, opt.size)
        )
...
def inspect_saved_model(opt):
    ...
    for source_class in range(opt.n_cls):
        ...
        for metric_id in range(len(anomaly_metrics_one_source)):
            ...
            # 添加边界检查，确保metric_id不会超过opt.n_cls
            if metric_id < opt.n_cls:
    ...
    # 保存矩阵数值到.npy文件（可用于后续加载和分析）
    np.save(f'./inspect_results/InspectResultMatrix--{id_str}.npy', anomaly_metric_value_array)
    # 也可以保存为CSV格式以便用Excel等工具打开查看
    np.savetxt(f'./inspect_results/InspectResultMatrix--{id_str}.csv', anomaly_metric_value_array, delimiter=',', fmt='%.4f')
    
    # 保存tendency_every_target_class数组
    np.save(f'./inspect_results/InspectResultTendency--{id_str}.npy', tendency_every_target_class)
    np.savetxt(f'./inspect_results/InspectResultTendency--{id_str}.csv', tendency_every_target_class, delimiter=',', fmt='%.4f')
    
    # 保存包含异常指标值的文本文件
    with open(f'./inspect_results/InspectResultMetric--{id_str}.txt', 'w') as f:
        f.write(f'Anomaly metric value: {_anomaly_metric}\n')
        f.write(f'Total classes: {opt.n_cls}\n')
        f.write('\nTendency for each target class:\n')
        for i, val in enumerate(tendency_every_target_class):
