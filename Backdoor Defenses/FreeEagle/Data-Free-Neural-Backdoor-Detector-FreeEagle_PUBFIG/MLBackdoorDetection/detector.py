...
from torchvision.datasets import GTSRB, ImageFolder  # 添加GTSRB数据集导入
...
# First, modify the Detector class initialization method to accept dataset parameter
class Detector:
    def __init__(self, model, images, attack_type, size_limit, k=4, image_labels=None, dataset=None):
        self.model   = model
        self.grid_rescale = 1
        self.k = k
        self.dataset = dataset  # Store dataset name
        # 存储图像原始类别
        ...
        if self.attack_type in ["blend", "lira", "semantic"]:
            ...
            # FIX: Use 256 instead of 2^resolution[dataset] to stay within int32 limits
            self.pattern      = np.repeat(np.random.randint(256, size=(1,resolution[dataset])), dimensions[dataset], axis=0)
    ...
    def optimize(self, target_label, num_rounds, lambd):
        ...
        # 计算初始模式的分数
        initial_images = self.apply_backdoor(self.pattern, self.top_left, self.bottom_right)
        if self.image_labels is not None:
            _, initial_score = self.compute_score(initial_images, target_label)
        else:
            initial_score = self.compute_score(initial_images, target_label)
        ...
        for round_num in range(num_rounds):
            ...
            if self.score >= max_score:  # 修改为 >= 以处理相等的情况
                max_score        = self.score
                max_pattern      = self.pattern
                max_top_left     = self.top_left
                max_bottom_right = self.bottom_right
    ...
    def flip_neighbor_pixels(self, coords):
        ...
        if self.attack_type in ["blend", "lira", "semantic"]:
            ...
            # 修改 flip_neighbor_pixels 方法中的位运算
            for channel,row,col in coords:
                if self.attack_type == "wanet":
                    ...
                    # 修改：使用8位范围内的位运算，因为我们的像素值在0-255之间
                    # 使用列索引的低8位来确定要翻转的位
                    bit_position = (resolution[dataset]-1-col) % 8  # 限制在位0-7范围内
                    neighbor_pattern[channel][row] = self.pattern[channel][row] ^ (1 << bit_position)
                    # 确保结果仍在0-255范围内
                    neighbor_pattern[channel][row] = neighbor_pattern[channel][row] & 0xFF
...
# 2. 修改main函数，实现从torchvision加载GTSRB数据集
if __name__ == "__main__":
    ...
    # 从torchvision加载GTSRB测试集，并为每个标签选择一个样本
    dataset_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "gtsrb")
    os.makedirs(dataset_dir, exist_ok=True)
    
    # 定义图像预处理转换
    transform = Compose([
        Resize((224, 224)),  # 调整为模型期望的输入尺寸
        ToTensor(),
        Normalize((0.55206233,0.44260582,0.37644434),(0.2515312,0.22786127,0.22155665))  # 标准化
    ])
    
    # # 加载GTSRB测试集
    # test_dataset = GTSRB(root=dataset_dir, split='test', transform=transform, download=True)

    # 加载GTSRB测试集
    test_dataset = ImageFolder(root="../../data/pubfig83_nonsmile", transform=transform)

    
    # 从每个标签中选择一个样本（确保模型预测与标签一致）
    selected_images = []
    selected_labels = []
    labels_found = set()
    ...
    for img, label in test_dataset:
        ...
        label_to_samples[label].append(img)
    
    # 对每个标签，找到模型能正确预测的样本
    for label in sorted(label_to_samples.keys()):
        samples = label_to_samples[label]
        for img in samples:
            # 将单个图像转换为批量格式并移至设备
            img_batch = img.unsqueeze(0).to(device)
            # 获取模型预测
            predicted = get_predicted_labels(model, img_batch, device)
            # 如果预测与实际标签一致，选择这个样本
        ...
        # 当找到所有83个标签的样本后停止
        if len(labels_found) == 83:
            break
    
    # 如果某些标签没有找到符合条件的样本，使用原逻辑兜底
    if len(labels_found) < 83:
        for img, label in test_dataset:
    ...
    for i in range(num_batches):
        ...
        # 将每个批次转换为张量
        batch_images = torch.stack(selected_images[start_idx:end_idx]).to(device)
        batch_labels = torch.tensor(selected_labels[start_idx:end_idx]).to(device)
    ...
    # image_labels = torch.tensor(selected_labels).to(device)
    ...
    # Then, update the Detector instantiation in the main function to pass the dataset parameter
    for label in range(num_labels[dataset]):
        ...
        best_score = -1
        best_class_scores = None
        
        # 处理每个批次 - 修改代码开始
        for batch_idx, (batch_images, batch_labels) in enumerate(zip(image_batches, label_batches)):
            label = 6
            print(f"Processing batch {batch_idx+1}/{num_batches} for target label {label}...")
            
            # Pass dataset to Detector constructor
            detector = Detector(model=model, images=batch_images, attack_type=args.attack_type, size_limit=args.size_limit, image_labels=batch_labels, dataset=dataset)
            ...
            if result["score"] > best_score:
                ...
                if "class_scores" in result:
                    best_class_scores = result["class_scores"]
        ...
        # 构造最终结果字典
        final_result = {
            "score": best_score,  # 使用最佳批次的得分
            "avg_score": avg_score,  # 添加平均得分
            "pattern": best_pattern,
            "top_left": best_top_left,
            "bottom_right": best_bottom_right,
            "runtime": runtime
        }
        
        if best_class_scores is not None:
            final_result["class_scores"] = best_class_scores
        
        print(f"\nHighest cASR = {best_score:.4f}")
        print(f"Average cASR = {avg_score:.4f}")
        print(f"Total runtime = {runtime:.2f}s\n")
        label_results[label] = final_result
        
        # 保存当前标签的trigger为图片和torch格式
        # 创建一个临时检测器用于保存结果
        temp_detector = Detector(model=model, images=image_batches[0], attack_type=args.attack_type, size_limit=args.size_limit, dataset=dataset)
        temp_detector.pattern = best_pattern
        temp_detector.top_left = best_top_left
        temp_detector.bottom_right = best_bottom_right
        save_trigger_as_image_and_torch(temp_detector, final_result, results_path, label, dataset)
        
        # 更新二维ASR矩阵
        if args.with_labels and "class_scores" in final_result:
            for source_cls, cls_score in enumerate(final_result["class_scores"]):
