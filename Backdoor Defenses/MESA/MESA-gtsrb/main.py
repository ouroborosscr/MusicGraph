...
from itertools import accumulate as _accumulate  # 修改这一行
...
def dataloader(dataset, bs=128):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=bs, shuffle=True, num_workers=0, drop_last=True, collate_fn=collate_fn)
    return dataloader

def dataset_split(dataset, ratio):
    from torch.utils.data import Subset  # 添加这行导入
...
def prepare_data(name, type_):
    ...
    # 为GTSRB数据集添加Resize变换，统一图像尺寸为32x32
    if name == 'GTSRB':
        transform = trans.Compose([
            trans.Resize((32, 32)),  # 添加这一行，将所有图像调整为32x32
            trans.ToTensor(),
    ...
    if name == 'cifar10':
        ...
        data_path = '../../data'
        # 修改这里，根据type_参数设置split
        split = "train" if type_ == 'train' else "test"
        dataset = torchvision.datasets.GTSRB(root=data_path, split=split, download=True, 
                                               transform=transform)
...
# 在plot_samples函数后添加
def save_samples_as_image(X, epoch):
    """将samples保存为图像文件"""
    h = w = int(math.sqrt(X.shape[1] / 3))
    assert h * w == X.shape[1] / 3
    n = 8 if h < 6 else 4
    ...
    # 将图像数据从0-1范围转换到0-255范围
    img = (np.clip(img, 0, 1) * 255).astype(np.uint8)
    
    # 保存图像
    img_path = f'saved_samples/trigger_samples_epoch_{epoch}.png'
...
class Model():
    def __init__(self, dataset_name):
        ...
        if dataset_name == 'cifar10':
            ...
            model_path = './model_last.pt.tar'
            self.net = GTSRBNet(num_classes=43)
            checkpoint = torch.load(model_path)
...
class BD():
    ...
    def train(self, dataset, num_epochs, poison_ratio, lr=1e-3): # one epoch in default
        ...
        for epoch in range(math.ceil(num_epochs)):
            ...
            # 使用tqdm包装loader并添加描述
            for batch_idx, (data, label) in enumerate(tqdm(loader, desc=f'Epoch {epoch+1}/{math.ceil(num_epochs)}')):
            ...
            # 添加训练精度显示
            tqdm.write(f'Train Accuracy: {correct/total:.4f}')
    ...
    def test(self, dataset, num_epochs, is_attack):
        ...
        with torch.no_grad():
            # 使用tqdm包装loader并添加适当的描述
            for batch_idx, (data, label) in enumerate(tqdm(loader, desc=f'Testing (Attack: {is_attack})')):
                data, label = data.clone().cuda(), label.clone().cuda()
                ...
                total += label.size(0)
                correct += predicted.eq(label).sum().item()
                non_target_total += torch.sum(torch.logical_not(ori_label.eq(target))).item()
                non_target_correct += (predicted.eq(label) * torch.logical_not(ori_label.eq(target))).sum().item()

        # 显示测试结果
        if is_attack == True:
            accuracy = non_target_correct / non_target_total
            tqdm.write(f'Test ASR: {accuracy:.4f}')
            self.log.append({'Test ASR': accuracy})
        else:
            accuracy = correct / total
            tqdm.write(f'Test Accuracy: {accuracy:.4f}')
            self.log.append({'Test Accuracy': accuracy})

    def exist(self):
        return os.path.isfile(self.filename)

    def load(self):
        ckpt = torch.load(self.filename, weights_only=False)
...
class GM():
    def __init__(self, BD_model, search_param):
        ...
        if BD_model.model.dataset_name == 'cifar10':
            ...
            self.lr = 0.0002
            self.num_epochs = 10
        elif BD_model.model.dataset_name == 'GTSRB':
            GM_param = {'in_size': self.in_size, 'out_size': 27, 'hidden_size': 512}
            self.print_interval = 10
            self.lr = 0.0002
            self.num_epochs = 1
    ...
    def search(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for idx, (data, label) in enumerate(loader):
                ...
                non_target_total += torch.sum(torch.logical_not(label.eq(target))).item()
                non_target_correct += (predicted.eq(target) * torch.logical_not(label.eq(target))).sum().item()
            ...
            # 添加保存torch格式的代码
            samples_cuda = self.sample(type_='cuda')  # 获取CUDA格式的samples
            
            # 确保保存目录存在
            os.makedirs('saved_samples', exist_ok=True)
            
            # 保存为torch格式
            torch_filename = f'saved_samples/trigger_samples_epoch_{epoch}.pt'
            torch.save(samples_cuda, torch_filename)
            print(f"已保存torch格式到: {torch_filename}")
            
            # 保存为图像格式
            save_samples_as_image(samples, epoch)
            
            if epoch % self.print_interval == 0:
                plot_samples(samples)
            self.log.append({'Trigger samples': samples.shape})
            self.log.append({'Trigger samples shape': samples.shape})
...
class Experiments():
    ...
    @staticmethod
    def test_contour_GTSRB(attack_param={'num_epochs':1, 'poison_ratio':0.01}):
    ...
    @staticmethod
    def test_alpha_GTSRB(attack_param={'num_epochs':10, 'poison_ratio':0.01}):
        trigger = Trigger({'name':'3x3binary', 'num':170})
        model = Model('GTSRB')
        BD_model = BD(model, trigger, attack_param)
        BD_model.try_load()
    ...
    @staticmethod
    def detect_GTSRB(attack_param={'num_epochs':10, 'poison_ratio':0.01}):
        trigger = Trigger({'name':'3x3binary', 'num':170})
        model = Model('GTSRB')
        BD_model = BD(model, trigger, attack_param)
        BD_model.try_load()
    ...
    def defend_GTSRB_10_all(attack_param={'num_epochs':10, 'poison_ratio':0.01}):
        ...
        for beta in [0.5,0.8,0.9]:
            ...
            file = open('GTSRB_results_'+str(beta)+'_'+trigger_name+'_rand.pickle', 'wb')
...
if __name__ == '__main__':
    Experiments.test_contour_GTSRB()