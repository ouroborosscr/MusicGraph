...
def optimize_trigger_for_neuron_local(state_dict_path, model_class, num_classes,
    ...
    for it in tqdm(range(iters), desc=f"Optimizing {layer_name} channel {channel_idx}"):
        optimizer.zero_grad()
...
def main(args):
    ...
    triggers_per_target = {}
    for t in tqdm(range(num_classes), desc="Processing targets"):
        tasks=[]
        ...
        res=list(tqdm(pool.imap(worker_optimize,tasks), total=len(tasks), desc=f"Processing candidates for target {t}"))
