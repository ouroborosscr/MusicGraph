...
class LeNet(nn.Module):
    ...
    def forward(self, x, latent=False):
        ...
        x = F.max_pool2d(x, 2, 2)
        if x.requires_grad:
            # 确保activations_hook方法存在
            if hasattr(self, 'activations_hook'):
...
def prepare_data(name, type_):
    ...
    if name == 'cifar10':
        ...
        if type_ == 'train':
            ...
            is_train = False
        dataset = torchvision.datasets.MNIST(root=data_path, train=is_train, download=True, 
                                             transform=transform)
...
class Model():
    def __init__(self, dataset_name):
        ...
        if dataset_name == 'cifar10':
            ...
            # 尝试加载模型权重，如果文件不存在则使用随机初始化的模型
            try:
                checkpoint = torch.load(model_path)
                self.net.load_state_dict(checkpoint['state_dict'])
                print("MNIST model loaded successfully")
            except FileNotFoundError:
                print("MNIST model file not found, using randomly initialized model")
