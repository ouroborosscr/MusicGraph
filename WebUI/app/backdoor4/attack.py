...
class Attack:
    ...
    def __init__(self, params, synthesizer, synthesizer2, synthesizer3, synthesizer4):
        self.params = params
        self.synthesizer = synthesizer
        self.synthesizer2 = synthesizer2
        self.synthesizer3 = synthesizer3
        self.synthesizer4 = synthesizer4
    ...
    def compute_blind_loss(self, model, criterion, batch, attack):
        ...
        batch_back2 = self.synthesizer2.make_backdoor_batch(batch, attack=attack)
        batch_back3 = self.synthesizer3.make_backdoor_batch(batch, attack=attack)
        batch_back4 = self.synthesizer4.make_backdoor_batch(batch, attack=attack)
        scale = dict()
        ...
        if len(loss_tasks) == 1:
            loss_values, grads = compute_all_losses_and_grads(
                loss_tasks,
                self, model, criterion, batch, batch_back, batch_back2, compute_grad=False
            )
        elif self.params.loss_balance == 'MGDA':

            loss_values, grads = compute_all_losses_and_grads(
                loss_tasks,
                self, model, criterion, batch, batch_back, batch_back2, batch_back3, batch_back4, compute_grad=True)
            if len(loss_tasks) > 1:
