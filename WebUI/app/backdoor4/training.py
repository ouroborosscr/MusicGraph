...
def test(hlpr: Helper, epoch, backdoor=False):
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(hlpr.task.test_loader)):
            batch2 = hlpr.task.get_batch(i, data)
            # print(batch2.labels)
            if backdoor:
                batch2 = hlpr.attack.synthesizer2.make_backdoor_batch(batch2,
                                                                    test=True,
                                                                    attack=True)
                # print(batch2.labels)
                # die()
            ...
            outputs2 = model(batch2.inputs)
            hlpr.task.accumulate_metrics(outputs=outputs2, labels=batch2.labels)
    metric2 = hlpr.task.report_metrics(epoch,
                             prefix=f'Backdoor {str(backdoor):5s}. Epoch: ',
                             tb_writer=hlpr.tb_writer,
                             tb_prefix=f'Test_backdoor_{str(backdoor):5s}')
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(hlpr.task.test_loader)):
            batch3 = hlpr.task.get_batch(i, data)
            # print(batch2.labels)
            if backdoor:
                batch3 = hlpr.attack.synthesizer3.make_backdoor_batch(batch3,
                                                                    test=True,
                                                                    attack=True)
                # print(batch2.labels)
                # die()
            ...
            outputs3 = model(batch3.inputs)
            hlpr.task.accumulate_metrics(outputs=outputs3, labels=batch3.labels)
    metric3 = hlpr.task.report_metrics(epoch,
                             prefix=f'Backdoor {str(backdoor):5s}. Epoch: ',
                             tb_writer=hlpr.tb_writer,
                             tb_prefix=f'Test_backdoor_{str(backdoor):5s}')
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(hlpr.task.test_loader)):
            batch4 = hlpr.task.get_batch(i, data)
            # print(batch2.labels)
            if backdoor:
                batch4 = hlpr.attack.synthesizer4.make_backdoor_batch(batch4,
                                                                    test=True,
                                                                    attack=True)
                # print(batch2.labels)
                # die()
            ...
            outputs4 = model(batch4.inputs)
            hlpr.task.accumulate_metrics(outputs=outputs4, labels=batch4.labels)
    metric4 = hlpr.task.report_metrics(epoch,
                             prefix=f'Backdoor {str(backdoor):5s}. Epoch: ',
                             tb_writer=hlpr.tb_writer,
                             tb_prefix=f'Test_backdoor_{str(backdoor):5s}')
    hlpr.task.reset_metrics()
    if epoch==1:
        print("final_metric1:",metric,end=',')
        print("final_metric2:",metric2,end=',')
        print("final_metric3:",metric3,end=',')
        print("final_metric4:",metric4,end='}')
    metric = metric+metric2+metric3+metric4
    metric/=4
